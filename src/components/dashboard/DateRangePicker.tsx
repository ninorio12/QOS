...
export function DateRangePicker({ onClose, onApply }: { onClose: () => void; onApply: (start: Date, end: Date, label: string) => void }) {
  ...
  return (
    <div
      className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 max-w-[calc(100vw-1rem)] max-h-[calc(100dvh-2rem)]
        md:absolute md:left-auto md:right-0 md:top-full md:mt-2 md:translate-x-0 md:translate-y-0 md:max-w-none md:max-h-none
        z-50 flex rounded-2xl shadow-2xl border border-[#E5E7EB] overflow-hidden"
