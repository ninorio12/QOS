...
function KpiCard({ label, value, sub, color }: { label: string; value: string; sub?: string; color: string }) {
  return (
    <div className="bg-white border border-[#E5E7EB] rounded-2xl px-4 py-3.5 flex-1">
    ...
    <div className="bg-white border border-[#E5E7EB] rounded-2xl p-4 flex items-center justify-between">
      <div>
        <div className="flex items-center gap-2 mb-0.5">
          ...
          {connected
            ...
            : <XCircle size={13} className="text-[#9CA3AF]" />
          }
        </div>
      ...
      <div className="text-right">
        <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${
          connected ? 'bg-[#22c55e]/15 text-[#22c55e]' : 'bg-[#3D4F6B]/20 text-[#9CA3AF]'
        }`}>
          {connected ? 'CONNECTÉ' : 'EN ATTENTE'}
        </span>
        {meta && <p className="text-[10px] text-[#9CA3AF] mt-1">{meta}</p>}
      </div>
    </div>
  )
}
      ...
      <div>
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-2xl overflow-hidden">
          {[
            ...
            <div key={row.label} className={`flex items-start justify-between px-5 py-3.5 gap-4 ${i < arr.length - 1 ? 'border-b border-[#E5E7EB]' : ''}`}>
              <span className="text-xs text-[#9CA3AF] flex-shrink-0 w-36">{row.label}</span>
              <span className="text-xs font-semibold text-[#111111] text-right">{row.value}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-2xl overflow-hidden">
          <div className="grid grid-cols-4 px-4 py-2.5 border-b border-[#E5E7EB]">
            {['Contact', 'Date', 'Durée', 'Résultat'].map(h => (
              <p key={h} className="text-[9px] font-bold uppercase tracking-wider text-[#9CA3AF]">{h}</p>
            ))}
          </div>
          {CALL_HISTORY.map((row, i) => {
            ...
            return (
              <div
                key={i}
                className={`grid grid-cols-4 px-4 py-3 items-center ${i < CALL_HISTORY.length - 1 ? 'border-b border-[#E5E7EB]' : ''}`}
              >
                <p className="text-xs font-semibold text-[#111111]">{row.contact}</p>
                <p className="text-xs text-[#9CA3AF]">{row.date}</p>
                <p className="text-xs text-[#6B7280]">{row.duration}</p>
                <span
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div>
        ...
        <div className="space-y-4">
          {templates.map((t, i) => (
            <div key={t.stage} className="bg-white border border-[#E5E7EB] rounded-2xl overflow-hidden">
              {/* header */}
              <div className="flex items-center justify-between px-4 py-3 border-b border-[#E5E7EB]">
                <div className="flex items-center gap-2">
                  <MessageSquare size={12} className="text-[#9CA3AF]" />
                  <span className="text-xs font-bold text-[#111111]">{t.stage}</span>
                </div>
                <button
                  ...
                  className={`text-[10px] font-bold px-2.5 py-1 rounded-full transition-colors ${
                    t.active
                      ...
                      : 'bg-[#3D4F6B]/20 text-[#9CA3AF] hover:bg-[#3D4F6B]/30'
                  }`}
                >
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div>
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-2xl p-4 flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold text-[#111111]">Configurer le chatbot</p>
            <p className="text-xs text-[#9CA3AF] mt-0.5">Scénarios, réponses et intégration</p>
          </div>
          <a
            href="#"
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-[#E5E7EB] text-xs font-medium text-[#6B7280] hover:text-[#111111] hover:border-[#3D4F6B] transition-colors"
          >
      ...
      <div>
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-2xl overflow-hidden">
          {CHATBOT_CONVS.map((c, i) => (
            <div
              key={i}
              className={`flex items-center justify-between px-4 py-3 ${i < CHATBOT_CONVS.length - 1 ? 'border-b border-[#E5E7EB]' : ''}`}
            >
              <div className="flex items-center gap-3">
                <div className="w-7 h-7 rounded-full bg-[#EEF0EB] flex items-center justify-center">
                  <Bot size={12} className="text-[#9CA3AF]" />
                </div>
                <div>
                  <p className="text-xs font-semibold text-[#111111]">{c.contact}</p>
...
export default function ConversionView() {
  ...
  return (
    <div className="p-6 bg-[#EEF0EB] min-h-[calc(100vh-56px)]">
      {/* Module tabs */}
      <div className="flex items-center gap-1 bg-white border border-[#E5E7EB] rounded-xl p-1 self-start w-fit mb-6">
        <Link href="/conversations" className="px-3 py-1.5 rounded-lg text-[12px] font-semibold text-[#6B7280] hover:text-[#111111] transition-all">
          Conversations
        </Link>
        <Link href="/conversion" className="px-3 py-1.5 rounded-lg text-[12px] font-semibold bg-[#111111] text-white transition-all">
          Conversion IA
        </Link>
      </div>
      ...
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-lg font-bold text-[#111111]">Conversion</h1>
        <p className="text-xs text-[#6B7280] mt-0.5">Canaux de conversion automatisés</p>
      </div>
