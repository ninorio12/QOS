...
const TOOLS: Tool[] = [
  ...
  // ── Cockpit Media Buyer (construit le 2026-07/08) : séries Meta via Zernio,
  //    moteur de décisions sur les créas, fils de discussion par card et leviers.
  { name: 'ads_sync_daily', description: "Resynchronise les séries JOURNALIÈRES Meta via Zernio (dépense, impressions, clics, leads) qui alimentent les KPI, les courbes et le tableau Détail. days (30 par défaut, 120 max) ou from/to (AAAA-MM-JJ). Zernio ignore le découpage natif : la reconstruction se fait jour par jour, donc une large fenêtre prend du temps.", inputSchema: obj({ days: Sx.number, from: Sx.string, to: Sx.string }), run: (a) => cx().action(api.zernioAds.syncDaily, { days: a.days || undefined, from: a.from || undefined, to: a.to || undefined }) },
  { name: 'ads_decisions_analyze', description: "Passe les créas au moteur de verdicts (couper, scaler, fatigue, variante) et dépose les propositions non encore vues. Règles chiffrées et explicables : seuils de dépense, CPA contre la médiane du compte, fréquence, hook et hold. Retourne le nombre de propositions créées.", inputSchema: obj({}), run: () => cx().mutation(api.adsIntel.analyze, {}) },
  { name: 'ads_decisions_open', description: "Propositions de décision en attente d'arbitrage humain (verdict, raison chiffrée, action proposée).", inputSchema: obj({}), run: () => cx().query(api.adsIntel.listOpen, {}) },
  { name: 'ads_decisions_history', description: "Décisions déjà tranchées (validées, refusées, modifiées), 50 dernières.", inputSchema: obj({}), run: () => cx().query(api.adsIntel.history, {}) },
  { name: 'ads_decisions_propose', description: "Dépose TA propre proposition de décision sur une pub, un adset ou une campagne. scope (ad|adset|campaign), refId, refName, verdict (scale|kill|watch|variant), reason (chiffrée), action (ce qu'il faut faire). L'humain tranche ensuite.", inputSchema: obj({ scope: Sx.string, refId: Sx.string, refName: Sx.string, verdict: Sx.string, reason: Sx.string, action: Sx.string, proposedBy: Sx.string }, ['scope', 'refId', 'refName', 'verdict', 'reason', 'action']), run: (a) => cx().mutation(api.adsIntel.propose, { scope: a.scope, refId: a.refId, refName: a.refName, verdict: a.verdict, reason: a.reason, action: a.action, proposedBy: a.proposedBy || 'agent' }) },
  { name: 'cockpit_cards_overview', description: "État des cards KPI du cockpit Media Buyer : dernier message de chaque fil de discussion, fils actifs (moins de 3 jours), nombre de messages archivés, et valeur courante des leviers (budget/jour, objectif leads, plancher CTR).", inputSchema: obj({}), run: () => cx().query(api.cardThreads.overview, {}) },
  { name: 'cockpit_card_post', description: "Écris un message dans le fil d'une card KPI du cockpit (spend|impressions|clicks|leads|cpl|ctr|cr|chart_cpl|chart_leads|top_campaigns|detail). Tu apparais avec le badge Agent. icon='target' quand tu poses une cible.", inputSchema: obj({ cardId: Sx.string, body: Sx.string, icon: Sx.string, author: Sx.string }, ['cardId', 'body']), run: (a) => cx().mutation(api.cardThreads.post, { cardId: a.cardId, body: a.body, icon: a.icon || undefined, author: a.author || undefined }) },
  { name: 'cockpit_card_history', description: "Messages archivés (plus de 3 jours) du fil d'une card KPI. cardId requis.", inputSchema: obj({ cardId: Sx.string }, ['cardId']), run: (a) => cx().query(api.cardThreads.history, { cardId: a.cardId }) },
  { name: 'cockpit_set_lever', description: "Bouge un levier du cockpit : budgetPerDay (CHF/jour), leadsWeekly (objectif hebdo), cplTarget, ctrFloor, freqMax. Le changement laisse un message ÉCRIT dans le fil de la card concernée, à ton nom. ⚠️ Ne bouge un budget qu'après validation humaine explicite.", inputSchema: obj({ key: Sx.string, value: Sx.number }, ['key', 'value']), run: (a) => cx().mutation(api.cardThreads.setLever, { key: a.key, value: a.value }) },
  { name: 'cockpit_synthese_get', description: "Synthèse en cours du module Media Buyer (note libre, éditable 24 h) et son auteur.", inputSchema: obj({}), run: () => cx().query(api.synthese.current, {}) },
  { name: 'cockpit_synthese_save', description: "Écris ou complète la synthèse du jour du module Media Buyer. Elle bascule automatiquement dans l'historique après 24 h.", inputSchema: obj({ body: Sx.string }, ['body']), run: (a) => cx().mutation(api.synthese.save, { body: a.body }) },
  { name: 'cockpit_synthese_history', description: "Synthèses passées du module Media Buyer, récentes d'abord.", inputSchema: obj({}), run: () => cx().query(api.synthese.history, {}) },
