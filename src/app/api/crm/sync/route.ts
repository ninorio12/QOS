...
export async function POST(req: NextRequest) {
  ...
  try {
    const body = await req.json().catch(() => ({})) as { contactId?: string; all?: boolean }
    ...
    const result = await c.mutation(api.sync.syncContactToPipeline, { contactId: body.contactId as Id<'crm_contacts'> })
    return NextResponse.json(result)
  } catch (err) {
