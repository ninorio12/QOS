...
function buildContractHtml(d: ContractData) {
  ...
  const foot = `<div class="footer">VIVIDFLOW.CH &nbsp;/&nbsp; CONTRAT ${d.ref}</div>`
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .dot { width: 7px; height: 7px; border-radius: 50%; background: #16181d; margin-left: auto; margin-top: 2px; }
...
export async function POST(req: NextRequest) {
  try {
    const b = await req.json() as {
      clientName: string; company?: string; address?: string; phone?: string; email?: string;
      representant?: string; amount: number; installments: number; amounts: number[];
      ref?: string; currency?: string; preview?: boolean
    }
    const now = new Date()
