    ...
    <div className="h-full flex flex-col overflow-hidden p-5 animate-pulse">
      <div className="flex items-center justify-between mb-4 flex-shrink-0">
        ...
        <div className="flex gap-2">
          <div className="h-9 w-36 bg-soren-border rounded-xl" />
      ...
      <div className="grid grid-cols-4 gap-3 mb-4 flex-shrink-0">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="bg-soren-card rounded-2xl p-4 border border-soren-border shadow-sm">
            <div className="h-2.5 w-20 bg-soren-border rounded mb-3" />
            <div className="h-7 w-14 bg-soren-border rounded-lg" />
          </div>
        ))}
      </div>
          ...
          <div key={i} className="bg-soren-card rounded-2xl p-4 border border-soren-border shadow-sm flex flex-col">
            ...
            <div className="flex-1 flex items-end gap-2 pb-2">
              {Array.from({ length: 8 }).map((_, j) => (
                <div
                  key={j}
                  className="flex-1 bg-soren-border rounded-t"
