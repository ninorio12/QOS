  ...
  return (
    <div className="h-full flex flex-col overflow-hidden animate-pulse">
      ...
      <div className="px-6 pt-6 pb-4 flex items-end justify-between flex-shrink-0">
        <div>
          <div className="h-7 w-20 bg-soren-border rounded-xl mb-1.5" />
          <div className="h-3 w-64 bg-soren-border rounded" />
        </div>
        <div className="flex gap-1 bg-soren-card rounded-xl shadow-sm p-1">
          {[48, 56, 64].map((w, i) => (
            <div key={i} className="h-7 rounded-lg bg-soren-border" style={{ width: w }} />
          ))}
        </div>
      </div>
      ...
      <div className="flex gap-3 px-6 mb-4 flex-shrink-0">
        {[
          ...
          <div key={i} className="flex-1 bg-soren-card rounded-2xl border border-soren-border shadow-sm px-4 py-3.5">
            <div className={`h-2.5 ${s.w} bg-soren-border rounded mb-2.5`} />
            <div className={`h-6 ${s.v} bg-soren-border rounded-lg`} />
          </div>
        ))}
      </div>
          ...
          <div className="flex flex-col">
            {Array.from({ length: 9 }).map((_, i) => (
              <div key={i} className="flex items-center gap-3 px-4 py-3 border-b border-[#F3F4F6] last:border-0">
                {/* Icon box */}
                <div className="w-9 h-9 rounded-xl bg-soren-border flex-shrink-0" />
                {/* Text */}
                <div className="flex-1">
                  <div className="h-2.5 bg-soren-border rounded mb-1.5" style={{ width: `${100 + (i * 23) % 80}px` }} />
                  <div className="h-2 bg-soren-border rounded" style={{ width: `${60 + (i * 17) % 60}px` }} />
                </div>
                {/* Cost */}
                <div className="h-3.5 w-14 bg-soren-border rounded flex-shrink-0" />
              </div>
            ))}
          </div>
        </div>
