...
- User is working inside an existing SaaS/Data OS and needs Claude Code or a developer to integrate the result. In that case, do not create an isolated throwaway HTML mockup as the main deliverable. First inspect or preserve the existing shell/modules, then produce an integration brief or component-level spec tied to the real routes/files/modules.
