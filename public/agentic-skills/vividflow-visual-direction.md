...
For short public/interface lines, avoid generic dashboard/business-control phrasing such as “Tout votre business, piloté depuis un seul espace.” It sounds SaaS-generic and too top-down. Prefer calm, concrete operational wording tied to daily work, tools, teams, and processes.

Validated replacement direction:
- Primary: “Tout votre quotidien opérationnel, réuni dans un seul espace.”
- Alternatives: “Vos outils, vos équipes et vos actions réunis dans un espace clair.” / “Un seul espace pour suivre, décider et avancer.”
...
## Live Vercel mockup / section-by-section DA workflow

When Jonathan/Thomas says the work is on Vercel, a live link, “c’est toujours le même”, “autre section”, or asks for a mock-up in an existing deployed page, do **not** generate a standalone image or unrelated asset by default. Treat it as a live project iteration:
1. Recover the existing Vercel URL/project/folder from the current conversation, session search, or `.vercel/project.json` before asking.
2. Modify the actual project files (`index.html`, CSS, React component, etc.) and redeploy the same Vercel project/alias when safe.
3. Preserve the already validated visual direction and only change the requested section.
4. For section mockups, deliver 3 distinct variants inside the page unless Jonathan asks for one final version.
5. If he says “pas gauche/droite” or “tout au centre”, use a centered composition: central stack, one focal media/image/video card, centered copy/CTA, no two-column layout.
6. Verify the deployed alias with HTTP/snippet and, when visual quality matters, screenshot/DOM QA before reporting done.

Pitfall: a polished generated image is still wrong if the user expected work inside the Vercel project. Ask for project context only after trying to recover it from session history and local folders.
...
For the R1 VividFlow presentation, load `references/r1-commercial-mechanics-calibration.md` and `references/r1-vividflow-copy-and-da-handoff-2026-06.md` before briefing Claude Code/Claude Design or storyboarding. Current correction: do **not** produce a theoretical 12-page agentique/IA education deck. Use the validated commercial mechanic: état des lieux → quotidien réel → friction → capacité IA connectée → système/mémoire → conséquence business → démo live. Current copy anchors include: `Toutes les entreprises veulent gagner du temps`, `Une demande arrive. Puis tout commence`, `Le temps se perd dans les passages`, `Vous n’avez pas besoin d’un outil de plus. Vous avez besoin que le travail circule mieux`, page 7 `Un système agentique fonctionne comme un corps vivant`, page 8 `Tout passe par la mémoire métier`, page 9 `Le vrai gain n’est pas le temps gagné. C’est ce que vous pouvez enfin en faire`, page 10 `Prenons une situation réelle`. Work section by section and do not advance before validation.
...
For Schmid-style section-by-section Claude Code handoffs and the latest projection-deck corrections, also load `references/outbound-projection-schmid-section-calibration.md`. It captures the split responsibilities (Hermès = copy/business reading, Claude Code = visual execution), exact section roles, section 2 copy formula, section 3 strict-copy lesson, section 4 bridge-to-plafond recalibration, and dossier fermé/ouvert corrections.

For the validated Schmid solution reveal and the next macro-understanding page, also load:
- `references/schmid-section-7-agent-conversation-simulation.md`
- `references/schmid-section-8-macro-understanding.md`
...
When creating or reviewing a VividFlow discovery/pitch deck, especially AGaaS / IAO client-call material, use `references/discovery-deck-calibration.md`, `references/discovery-deck-progressive-interaction.md`, and `references/discovery-deck-white-sketch-calibration.md`. For Claude Cotte / minimal editorial AGaaS deck reviews, also load `aga-as-handdrawn-deck` and its `references/session-2026-05-19-claude-cotte-kalvi-review.md`: keep the validated cream/editorial DA, then add Kalvi substance, diagrams, progressive moments, and phrase-level fixes. For R1 pre-sale discovery/demo/audit decks, also use `references/r1-agaas-sales-deck-brainstorming.md`, `references/r1-keynote-opening-jobs-calibration.md`, and `references/r1-commercial-mechanics-and-organism-metaphor.md` before any storyboard or prototype. Key lesson: do not produce a framed PowerPoint-like HTML deck and do not start from design/code/storyboard before extracting Jonathan's sales process. If Jonathan stops the direction as too theoretical, reset to the commercial mechanics: état des lieux → quotidien réel → friction → capacité IA → chaîne transformée → résultat → explication → démo. Do not make the R1 a long agentique course or an audit deck. Current R1 correction: start from `Toutes les entreprises veulent gagner du temps`, then show `Une demande arrive. Puis tout commence`, `Le temps se perd dans les passages`, and `Vous n’avez pas besoin d’un outil de plus. Vous avez besoin que le travail circule mieux`. Scelled later anchors: page 7 uses `Un système agentique fonctionne comme un corps vivant` (infrastructure, not isolated agents); page 8 uses `Tout passe par la mémoire métier` with the R2-style memory diagram; page 9 is a consequence/new posture page before demo, not an inspirational quote. For R1 openings, avoid starting from logos, dashboards, chat windows, or “AI deck” aesthetics; first find the business-first reveal moment. The earlier pattern `Une entreprise avance quand trois choses tiennent ensemble` → `Répondre / Exécuter / Coordonner` is a useful thinking aid, but should not override the more concrete “demande → contexte → action → outil → suivi/validation” chain when Jonathan asks for a sales-ready R1. Emulate the subtle old VividFlow horizontal experience (`https://vividflow-deckfr.netlify.app/`) when relevant: full-screen flow, no slide numbers/internal labels, strong but sparse copy, diagrams that live in the space, and a narrative built from client identification before AGaaS education. For the white-sketch AGaaS education deck specifically, do **not** apply the orange/black SaaS/social DA: use a light paper background, animated minimal sketches, no visible VividFlow logo, and no visible deck buttons unless Jonathan asks. Critical interaction lesson: screens should often reveal buttons/layers progressively, not show all ideas at once. Before any storyboard or design, run the sales-brainstorm roadmap from `references/discovery-deck-calibration.md`: one simple question at a time, visible bloc/question numbering, challenge vague answers without inventing intent, and extract Jonathan's actual R1 sales process first. If reference-led sketch prototypes are rejected or Jonathan asks for a handoff/stack for another builder, stop polishing the prototype and use `references/agaas-deck-external-handoff-calibration.md` to package references, stack, narrative, DA constraints, structure, and acceptance criteria for transfer.

For R1 DA production specifically: preserve the accepted base (off-white, sans-serif, black/grey/orange, Schmid/R2 sobriety) but do not settle for a “beautiful text slide”. Add communication visuals that prove the idea: flows, passages, operational chains, memory links, and business repositioning. Use Claude Code for system graphics, animation, integration, and page transitions; use ChatGPT/GPT image for fixed premium illustrations when needed (especially the page 7 organism/corps vivant asset). Do not over-illustrate every page: default pages 1–6 to animated diagrams/flows, page 7 to one memorable fixed illustration, page 8 to the R2-style memory diagram, page 9 to a left/right consequence composition, page 10 minimal.

For the latest R1 page-by-page calibration, load `references/r1-pages-1-3-visual-calibration-2026-06.md`. Key lessons: once a page direction is validated, do not touch it in advance; page 1 B is validated with restrained background task noise; page 2 uses a large “Nouvelle demande” card and the title `Une nouvelle demande n’arrive jamais seule.`; page 3 is validated around `Le temps se perd dans les passages.` with 5 action labels: `recevoir la demande → comprendre le besoin → chercher l’info → attendre le retour → relancer` (more relatable than abstract nouns or dry single verbs). Earlier page 4 (`Vous n’avez pas besoin d’un outil de plus...`) and page 5 (`La bonne IA ne répond pas seulement...`) became suspect when Thomas/Jonathan identified repetition around “gagner du temps”; do not defend them as validated if the active discussion is structure reduction.

For the corrected R1 opening structure, load `references/r1-opening-structure-calibration-2026-06.md`. Current lesson: the issue is not always a missing transition after page 3; it can be that pages 1–5 repeat the same time-saving angle. Prefer a short opening before `Un système agentique fonctionne comme un corps vivant`: **miroir de routine → friction réelle entre étapes → nécessité d’un système**. Start with the mirror if debating “cas réels” vs “miroir”: it creates immediate identification and avoids another explanatory preamble. Avoid using standalone pages for `Toutes les entreprises veulent gagner du temps`, `Vous n’avez pas besoin d’un outil de plus`, `La bonne IA ne répond pas seulement`, or `Pas un chatbot...` when the deck needs to be tighter.
