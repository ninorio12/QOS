...
When cloning a profile from another named profile, use `--clone-from <source_profile>`. Do **not** use `--clone <source>`: `--clone` is a flag and the source name will be parsed as an unexpected argument. Example: `hermes profile create vividflow-slack-extension --clone-from chief-of-staff`.
...
When Jonathan says Hermes was updated on the VPS and asks you to be “à jour”, verify the live runtime instead of relying on memory: version, active profile, provider/model/context, gateways, repo freshness, and current CLI command surface. Use `references/vps-hermes-current-state-audit.md`. Keep the answer short and redact any key fingerprints from `hermes status --all`.
...
### Context window exceeded / compression loops

If Hermes/Codex shows errors like `Your input exceeds the context window`, `Context length exceeded — stepping down`, or token bars such as `271K/128K`, the blocker is not usually the timer/service itself. The model provider is refusing the request because the session or job is still above the model context limit after compression.

Fast diagnosis:

```bash
systemctl --user status <service> --no-pager | head -70
journalctl --user -u <service> -n 120 --no-pager
ps aux | grep -E 'hermes|codex|curator' | grep -v grep
```

Operational fix:

- Start a fresh session (`/new` or `/clear`) instead of continuing the overloaded one.
- Do not paste full logs/HTML/session dumps back into the prompt.
- For background curators, limit inputs before the model call: exclude logs/cache/raw recovered sessions/HTML dumps, chunk by source, and target well under the model limit (e.g. 80k–100k for a 128k model).
- If a service is stuck compressing for a long time, stop it, inspect its script, add size guards, then restart the timer.
...
**WhatsApp / Baileys channel:** before building or reviving a custom OpenWA/Baileys bridge for a Hermes-backed WhatsApp pilot, check the native Hermes WhatsApp bridge first: `hermes whatsapp` pairs a Baileys-based WhatsApp Web session and `hermes gateway` runs it. Use `references/hermes-native-whatsapp-baileys.md` for env/config, access-control gotchas, and anti-ban framing. If messages do not get received/replied to, verify `WHATSAPP_ALLOWED_USERS`/`WHATSAPP_ALLOW_ALL_USERS` first; with no access control, Hermes denies incoming messages by design. For an existing OpenWA stack that must be revived, use `references/openwa-whatsapp-hermes-bridge.md`: OpenWA API on VPS → local webhook bridge → Hermes CLI/profile → OpenWA `send-text`. Keep the webhook local if OpenWA and Hermes run on the same VPS, run the bridge under `systemd --user`, include anti-loop filters (`fromMe`, groups, status, duplicates), and verify the Hermes target profile with a direct non-interactive `hermes --profile <profile> chat -Q ...` before promising a live WhatsApp agent. For OpenWA vs custom Baileys decisions, and especially recently restricted/unrestricted WhatsApp accounts, use `references/whatsapp-self-hosted-stack-selection.md`: reuse only as a low-volume pilot, not production/outbound automation. If Jonathan asks whether people use Hermes WhatsApp intensively, use `references/hermes-whatsapp-intensive-use-research.md`: public evidence found so far supports experimentation/pilots, not a confident high-volume production claim; Meta Cloud API/provider bridge remains the safer client/intensive direction.
...
- **Gateway typing indicator across bots**: for Telegram/Hermes bots, a single `send_typing` call is too short for long tool runs. Implement/verify a persistent typing task in `gateway/run.py` around `_run_agent`: start `adapter.send_typing(source.chat_id, metadata=thread_metadata)` immediately, refresh about every 4 seconds while the turn is running, cancel the task when the final response is ready, and call `stop_typing` when supported. On error, cancel the same task before sending the error response. Keep typing failures cosmetic: log/suppress them, never fail the agent turn because a platform rejected a chat action.
