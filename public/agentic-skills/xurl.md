...
3. If auth is missing entirely, stop and direct the user to the "One-Time User Setup" section — do NOT attempt to register apps or pass secrets yourself. For monitoring/veille tasks, continue with public web/GitHub/HN/arXiv fallbacks instead of blocking the report. For a public X article/post URL, try browser access and extract `document.body.innerText`; X login walls may still expose the article body, stats, and author context. Scroll/snapshot if needed, then clearly label anything inaccessible instead of claiming a full read.
...
### VividFlow / Thomas AI watch reports

When using X/web monitoring to brief Thomas or Jonathan on AI/agent news, keep the report short and decision-oriented. Start with one verdict line: `🔴 EN RETARD`, `🟠 ATTENTION`, `🟢 À JOUR`, or `🚀 EN AVANCE`, followed by why. Then include only: 1-3 meaningful changes with links, 1-3 concrete actions to add/test with “ça permettrait de…”, impact on VividFlow/Hermes/Brvndlab, and one line of ignored noise. Do not produce exhaustive long-form market reports unless explicitly asked.
