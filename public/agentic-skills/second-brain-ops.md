...
- When Jonathan says the Telegram flow should already be in Second Brain raw and organized in the library, but it feels like it is not working, use `references/vividflow-telegram-raw-to-curated-wiki.md`: distinguish raw capture from quarantine candidates and curated wiki promotion; if ingest is `quarantine_only`, update the cron so an agent curates validated signals into wiki pages rather than declaring the system healthy.
