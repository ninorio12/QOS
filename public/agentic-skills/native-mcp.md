...
When wrapping a third-party SDK as a local MCP server, prefer a small stdio server that reads credentials from `env`, exposes a `health` tool that only reports booleans, and keeps side-effecting tools clearly named. For Framer Server API specifics (project-bound API keys, `framer-api`, publish/deploy, collections), see `references/framer-server-api-mcp.md`.
